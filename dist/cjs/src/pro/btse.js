...
class btse extends btse$1["default"] {
    ...
    handleError(client, message) {
        ...
        if (code !== undefined) {
            if (code === 4001) {
                throw new errors.BadSymbol(errorMessage);
            }
            else if (code === 4003) {
                throw new errors.AuthenticationError(errorMessage);
            }
            else if (code === 4004) {
                throw new errors.ExchangeError(errorMessage); // RateLimitExceeded
            }
    ...
    handleOHLCV(client, message) {
        ...
        if (storedArray === undefined) {
            ...
            storedArray = new Cache.ArrayCacheByTimestamp(limit);
     ...
     */
    async watchPositions(symbols = undefined, since = undefined, params = {}) {
        await this.loadMarkets();
        await this.authenticate();
        symbols = this.marketSymbols(symbols, undefined, false);
        const url = this.getUrl(true);
