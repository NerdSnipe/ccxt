 ...
 * @augments Exchange
 */
class btse extends btse$1["default"] {
    nonce() {
        ...
        return this.deepExtend(super.describe(), {
            ...
            'exceptions': {
                'exact': {
                    ...
                    'SYMBOL_NOT_FOUND': errors.BadSymbol,
                    'INSUFFICIENT_BALANCE': errors.InsufficientFunds,
                    'INSUFFICIENT_MARGIN': errors.InsufficientFunds,
                    'INVALID_ORDER_SIZE': errors.InvalidOrder,
                    'INVALID_PRICE': errors.InvalidOrder,
                    'ORDER_SIZE_TOO_SMALL': errors.InvalidOrder,
                    'ORDER_SIZE_TOO_LARGE': errors.InvalidOrder,
                    'PRICE_TOO_HIGH': errors.InvalidOrder,
                    'PRICE_TOO_LOW': errors.InvalidOrder,
                    'ORDER_NOT_FOUND': errors.OrderNotFound,
                    'ORDER_ALREADY_FILLED': errors.OrderNotFound,
                    'ORDER_ALREADY_CANCELLED': errors.OrderNotFound,
                    ...
                    'API_KEY_EXPIRED': errors.AuthenticationError,
                    'PERMISSION_DENIED': errors.PermissionDenied,
                    'IP_RESTRICTED': errors.PermissionDenied,
                    'MARKET_CLOSED': errors.InvalidOrder,
                    'MAINTENANCE_MODE': errors.ExchangeNotAvailable,
                    'INVALID_NONCE': errors.InvalidNonce,
                    'DUPLICATE_ORDER_ID': errors.DuplicateOrderId,
                },
                'broad': {
                    'rate limit': errors.RateLimitExceeded,
                    'too many': errors.RateLimitExceeded,
                    ...
                    'signature': errors.AuthenticationError,
                    'maintenance': errors.ExchangeNotAvailable,
                },
            },
    ...
    sign(path, api = 'public', method = 'GET', params = {}, headers = undefined, body = undefined) {
        ...
        if (isPublic) {
            ...
            const signature = this.hmac(this.encode(auth), this.encode(this.secret), sha512.sha384, 'hex');
            headers = {
    ...
    async fetchMarkets(params = {}) {
        ...
        const promises = [
            this.publicGetApiV23MarketSummary(params),
    ...
    async cancelOrder(id, symbol = undefined, params = {}) {
        if (symbol === undefined) {
            throw new errors.InvalidOrder(this.id + ' cancelOrder() requires a symbol argument');
        }
        await this.loadMarkets();
        const market = this.market(symbol);
        const request = {
    ...
    async cancelAllOrders(symbol = undefined, params = {}) {
        if (symbol === undefined) {
            throw new errors.InvalidOrder(this.id + ' cancelAllOrders() requires a symbol argument');
        }
        await this.loadMarkets();
        const market = this.market(symbol);
        const request = {
    ...
    async editOrder(id, symbol, type, side, amount = undefined, price = undefined, params = {}) {
        ...
        if (price !== undefined && amount !== undefined && triggerPrice !== undefined) {
            ...
            throw new errors.InvalidOrder(this.id + ' editOrder() requires a price, amount, or triggerPrice argument');
        }
    ...
    async fetchOrder(id, symbol = undefined, params = {}) {
        ...
        throw new errors.OrderNotFound(this.id + ' fetchOrder() could not find order ' + id);
    }
    /**
     * @method
    ...
    async setLeverage(leverage, symbol = undefined, params = {}) {
        if (symbol === undefined) {
            throw new errors.InvalidOrder(this.id + ' setLeverage() requires a symbol argument');
        }
        await this.loadMarkets();
        const market = this.market(symbol);
        const request = {
